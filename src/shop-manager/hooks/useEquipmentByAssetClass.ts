...
import { Equipment, EquipmentStats } from '@sm/types/equipment';

export type AssetClass = 'shop_equipment' | 'fleet' | 'safety';

export function useEquipmentByAssetClass(assetClass?: AssetClass) {
  ...
  const fetchEquipment = async () => {
    try {
      console.log("Fetching equipment data for asset_class:", assetClass || 'all');
      ...
      // Filter by asset_class if provided
      if (assetClass) {
      ...
      console.log("Equipment data loaded:", equipmentData.length, "items for", assetClass || 'all');
      setError(null);
    } catch (err) {
