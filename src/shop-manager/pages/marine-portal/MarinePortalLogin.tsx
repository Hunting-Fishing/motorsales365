...
import { Loader2, LogIn, Ship } from 'lucide-react';

export default function MarinePortalLogin() {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      if (session?.user) {
        ...
        if (customer) { navigate('/marine-portal/dashboard'); return; }
      }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!customer) {
        ...
        toast({ title: "Access Denied", description: "No customer account found. Please register or contact your marine service provider.", variant: "destructive" });
        setLoading(false); return;
      }
      toast({ title: "Welcome Back!", description: `Signed in as ${customer.first_name} ${customer.last_name}` });
      navigate('/marine-portal/dashboard');
    } catch (error: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="shadow-xl bg-card/95 backdrop-blur-sm border-border/50">
          <CardHeader className="text-center space-y-2">
            <div className="mx-auto w-16 h-16 bg-indigo-700 rounded-full flex items-center justify-center mb-2"><LogIn className="h-8 w-8 text-white" /></div>
            <CardTitle className="text-2xl font-bold">Welcome Back</CardTitle>
            <CardDescription>Sign in to manage your vessel service</CardDescription>
          </CardHeader>
          <form onSubmit={handleLogin}>
            ...
            <CardFooter className="flex flex-col gap-4">
              ...
              <div className="text-center text-sm text-muted-foreground">
                ...
                <Link to="/marine-portal/register" className="text-indigo-700 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium">Register here</Link>
              </div>
            </CardFooter>
          </form>
        </Card>
