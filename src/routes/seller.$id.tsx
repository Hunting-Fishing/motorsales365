    ...
    const load = async () => {
      ...
      const [{ data: p }, { data: ls }, { data: rs }] = await Promise.all([
        ...
        supabase
          .from("listings")
          .select(
            "id,title,price_php,price_kind,negotiable,price_hidden,registration_status,region,city,seller_type,boost_until,status,category_slug,view_count,attributes,listing_media(url,type),vehicles:vehicle_id(is_public,passport_slug,vehicle_passport_verifications(status))",
          )
          .eq("user_id", id)
