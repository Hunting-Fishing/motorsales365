    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data: links, error } = await supabaseAdmin
          ...
          .select("id, url, product_id, last_checked_at, product:shop_products(id, price_php, deal_price_php, is_deal, active)")
        ...
        for (const link of links ?? []) {
          ...
          try {
            ...
            // Always stamp last_checked_at so we round-robin through items.
            await supabaseAdmin
              .from("shop_product_links")
              .update({ last_checked_at: now })
              .eq("id", link.id);

            if (!data || (!data.price && !data.sale_price)) continue;
