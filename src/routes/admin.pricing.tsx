...
function AdminPricing() {
  ...
  const runVerify = async () => {
    ...
    try {
      ...
      if (res.ok) toast.success("All paid plans resolve to active Stripe prices.");
        ...
        toast.error("Some plans are missing or inactive in Stripe — checkout will fail for those.");
    } catch (e: any) {
  ...
  const runSetTaxCodes = async () => {
    ...
    try {
      ...
      toast.success(
        `Tax codes: ${set} set, ${already} already set${failed ? `, ${failed} failed/missing` : ""}.`,
      );
    } catch (e: any) {
  ...
  const loadSubs = async () => {
    ...
    const [{ data: profs }, { data: pls }] = await Promise.all([
      ...
      planIds.length
        ? supabase.from("subscription_plans").select("id, name, price_php").in("id", planIds)
        : Promise.resolve({ data: [] as any[] }),
    ]);
    ...
    setSubs(
      list.map((r: any) => ({
        ...
        subscription_plans: planMap[r.plan_id],
      })),
    );
  };
  ...
  useEffect(() => {
    loadSubs(); /* eslint-disable-next-line */
  }, [subFilter]);
  ...
  const addPromo = async () => {
    ...
    const { error } = await supabase
      .from("promotions")
      .insert({ code: newPromo.code, percent_off: newPromo.percent_off, applies_to: "any" });
    if (error) toast.error(error.message);
    else {
      toast.success("Promo added");
      setNewPromo({ code: "", percent_off: 10 });
      load();
    }
  };
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Adjust fees, durations, plans, and promo codes — changes apply instantly.
        </p>
      </div>
      ...
      <section className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="space-y-3">
          {plans.map((p) => (
            <div
              key={p.id}
              className="grid gap-3 rounded-lg border border-border p-4 sm:grid-cols-[1fr_120px_140px_100px_auto] sm:items-end"
            >
              <div>
                <Label>Name</Label>
                <Input
                  defaultValue={p.name}
                  onBlur={(e) => savePlan(p.id, { name: e.target.value })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  defaultValue={p.listings_per_month ?? ""}
                  placeholder="∞"
                  onBlur={(e) =>
                    savePlan(p.id, {
                      listings_per_month: e.target.value ? Number(e.target.value) : null,
                    })
                  }
                />
              </div>
      ...
      <section className="rounded-xl border border-border bg-card p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Confirms every paid plan's lookup key resolves to an active price in Stripe (
              {getStripeEnvironment()}). Run this before announcing pricing changes — missing keys
        ...
        {verifyRows && (
          <div className="space-y-2">
            <div
              className={`rounded-md border p-2 text-sm ${verifyAllOk ? "border-emerald-500/40 bg-emerald-500/10" : "border-destructive/40 bg-destructive/10"}`}
            >
              {verifyAllOk
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {r.lookupKey && (
                      <code className="text-xs text-muted-foreground">{r.lookupKey}</code>
                    )}
                  </div>
      ...
      <section className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setNewPromo({ ...newPromo, percent_off: Number(e.target.value) })}
              className="w-24"
            />
          </div>
        ...
        <div className="mt-4 space-y-2">
          {promos.map((p) => (
            ...
            >
              <div>
                <span className="font-mono font-semibold">{p.code}</span> — {p.percent_off}% off
              </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold">
                        {prof?.full_name || s.user_id.slice(0, 8)}
                      </span>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {plan?.name ?? "Plan"} · {plan ? formatPHP(plan.price_php) + "/mo" : ""} ·
                      requested {formatDate(s.created_at)}
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {s.status === "active" && (
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          updateSub(s.id, { status: "paused", paused_at: new Date().toISOString() })
                        }
                      >
                        Pause
                      </Button>
                    ...
                    {s.status === "paused" && (
                      <Button
                        size="sm"
                        onClick={() => updateSub(s.id, { status: "active", paused_at: null })}
                      >
                        Resume
                      </Button>
                    )}
