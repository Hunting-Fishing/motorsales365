                ...
                <tr key={c.id} className="border-t">
                  ...
                  <td className="px-3 py-2 text-right">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={async () => {
                        if (!(await confirm({ title: `Delete "${c.title}"?`, destructive: true })))
                          return;
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{courseId ? "Edit course" : "New course"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <Field label="Title">
            <Input
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Stripe price lookup_key (one-time)">
              <Input
                ...
                placeholder="course_xxx_onetime"
              />
            </Field>
          ...
          <Field label="Included in subscription tiers (comma-separated plan names)">
            <Input
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  included_in_tiers: e.target.value
                    .split(",")
                    .map((s) => s.trim())
                    .filter(Boolean),
                })
            ...
            onClick={async () => {
              await upsert({
                data: {
                  ...form,
                  id: courseId,
                  price_php: form.price_php ?? undefined,
            ...
            <tbody>
              {(data?.partners ?? []).map((p: any) => (
                <tr key={p.id} className="border-t">
                  ...
                  <td className="px-3 py-2">
                    <Badge variant={p.tier === "featured" ? "default" : "outline"}>{p.tier}</Badge>
                  </td>
                  ...
                  <td className="px-3 py-2 text-right">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={async () => {
                        if (!(await confirm({ title: `Delete ${p.name}?`, destructive: true })))
                          return;
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[85vh] max-w-xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{initial ? "Edit partner" : "New partner"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <Field label="Name">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Sponsored until">
              <Input
                type="date"
                value={form.sponsored_until ?? ""}
                onChange={(e) => setForm({ ...form, sponsored_until: e.target.value })}
              />
            </Field>
          </div>
