  ...
  head: () => ({
    meta: [
      { title: "Pricing & plans — 365 MotorSales Philippines" },
      {
        name: "description",
        content:
          "Listing fees, upgrades, boosts, and monthly subscriptions for sellers in the Philippines.",
      },
    ],
  }),
      ...
      <section className="bg-secondary/40 py-12">
        <div className="container mx-auto px-4 text-center">
          <h1 className="font-display text-3xl font-bold sm:text-4xl">
            Free to post. Powerful for dealers.
          </h1>
          <p className="mt-2 text-muted-foreground">
            Posting a vehicle is always free for private sellers. Upgrade only when you need more
            reach, more listings, or business tools.
          </p>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 pb-16">
        ...
        {mySub && (
          <p className="mb-6 text-center text-sm text-muted-foreground">
            You have a <span className="font-semibold uppercase">{mySub.status}</span> subscription
            on file.{" "}
        ...
        {user && (
          <div className="mx-auto mb-8 max-w-3xl rounded-2xl border border-border bg-card p-6">
            ...
            {planLimit !== null && <Progress value={usagePct} className="mt-2 h-2" />}
            ...
            {(() => {
              ...
              const overran = usage.some((m) => planLimit !== null && m.count > (planLimit ?? 0));
              const wouldFit = upsellLimit === null || usage.every((m) => m.count <= upsellLimit);
              if (!overran && (upsellLimit === null || thisMonthCount * 2 <= upsellLimit))
                return null;
              return (
                <div className="mt-5 flex items-start gap-3 rounded-lg border border-primary/30 bg-primary/5 p-3 text-sm">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      {upsell.max_photos_per_listing ?? "more"} photos per listing for{" "}
                      {formatPHP(upsell.price_php)}/mo.
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {plans.map((p) => {
            ...
            const cmp =
              currentPlan && !isCurrent
                ? (() => {
                    ...
                    const photoDelta =
                      (p.max_photos_per_listing ?? 0) - (currentPlan.max_photos_per_listing ?? 0);
            ...
            const upgradeNet =
              kind === "upgrade" ? Math.max(0, (p.price_php ?? 0) - proratedCredit) : 0;
        ...
        <DialogContent>
          {confirmPlan &&
            (() => {
              ...
              const due = kind === "upgrade" ? upgradeNet : Number(plan.price_php) || 0;
              const title =
                kind === "upgrade"
                  ...
                  <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-4 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold">
                        {plan.name} — {formatPHP(plan.price_php)}/mo
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    {kind === "downgrade" && (
                      <p className="text-xs text-muted-foreground">
                        Takes effect immediately. Any unused portion of your current plan is
      ...
      <Dialog open={checkoutOpen} onOpenChange={(o) => !o && closeCheckout()}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Your card will be saved securely for future renewals. You can manage it any time from
              billing.
            </DialogDescription>
          </DialogHeader>
...
function DeltaRow({
  ...
  const tone = isZero ? "text-muted-foreground" : good ? "text-emerald-600" : "text-amber-600";
