  ...
  head: () => ({
    meta: [
      { title: "All shop categories — Detailing, Tools, Parts | 365 MotorSales" },
      {
        name: "description",
        content:
          "Browse every shop category grouped by department — performance parts, maintenance & fluids, repair & replacement, wheels, tools and more.",
      },
      ...
      <div className="container mx-auto space-y-10 px-4 py-8">
        ...
        <header className="space-y-2">
          ...
          <p className="max-w-2xl text-muted-foreground">
            Browse every section of the shop, grouped by department. Click a department for the full
            lander, or jump straight to a category.
          </p>
        </header>
              ...
              <section key={department.slug} className="space-y-4">
                <div className="flex items-end justify-between gap-3 border-b pb-2">
                  <Link
                    ...
                    className="group"
                  >
                    <h2 className="font-display text-xl tracking-tight group-hover:text-primary sm:text-2xl">
                      {department.name}
                    </h2>
                  </Link>
                  <Link
                    ...
                    <div key={parent.id} className="rounded-xl border bg-card p-5">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge variant="secondary" className="text-[10px]">
                          {parent.product_count} item{parent.product_count === 1 ? "" : "s"}
                        </Badge>
                      </div>
