...
function FavoritesPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("favorites")
      .select("listing_id, listings:listing_id(id,title,price_php,region,city,seller_type,boost_until,status,category_slug,user_id,view_count,listing_media(url,type),profiles:user_id(verification_status))")
      .eq("user_id", user.id)
      .then(({ data, error }) => {
          ...
          {items.map((l) => (
            <div key={l.id} className="relative group">
              ...
              <Button
                ...
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); removeFavorite(l.id); }}
