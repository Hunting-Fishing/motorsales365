  ...
  notFoundComponent: () => (
    <SiteLayout>
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <Link to="/rides" className="mt-4 inline-block text-primary hover:underline">Browse all rides</Link>
      </div>
    </SiteLayout>
  ),
...
function RideProfilePage() {
  ...
  const load = async () => {
    setLoading(true);
    const { data: r } = await (supabase as any).from("rides").select("*").eq("slug", slug).maybeSingle();
    ...
    const [{ data: p }, { data: m }, { data: l }, { data: o }, { data: prof }] = await Promise.all([
      ...
      (supabase as any).from("ride_mods").select("*").eq("ride_id", r.id).order("category").order("sort_order"),
      (supabase as any).from("ride_service_log").select("*").eq("ride_id", r.id).order("service_date", { ascending: false }),
    ...
    if (user) {
      const { data: lk } = await (supabase as any)
        .from("ride_likes").select("ride_id").eq("ride_id", r.id).eq("user_id", user.id).maybeSingle();
      setLiked(!!lk);
    }
    setLoading(false);
  };
  ...
  const handleLike = async () => {
    ...
    try { await likeFn({ data: { ride_id: ride.id } }); }
    catch (e: any) { setLiked(prev); setLikeCount((c) => c + (prev ? 1 : -1)); toast.error(e.message); }
  };

  const specs: Array<[string, any]> = [
    ["Year", ride.year], ["Make", ride.make], ["Model", ride.model], ["Trim", ride.trim],
    ["Color", ride.color], ["Engine", ride.engine], ["Transmission", ride.transmission],
    ["Drivetrain", ride.drivetrain], ["Mileage", ride.mileage_km ? `${ride.mileage_km.toLocaleString()} km` : null],
    ...
    <SiteLayout>
      ...
      <div className="relative">
        <div className="relative h-[40vh] min-h-[300px] w-full overflow-hidden bg-secondary">
          {ride.cover_photo_url ? (
            <img src={ride.cover_photo_url} alt={ride.name} className="h-full w-full object-cover" />
          ) : null}
          ...
          <div className="absolute inset-x-0 bottom-0 p-4 sm:p-8">
            <div className="container mx-auto">
              <div className="flex flex-wrap items-end justify-between gap-4 text-white">
                <div>
                  {ride.is_for_sale && (
                    <Badge className="mb-2 bg-primary"><Tag className="mr-1 h-3 w-3" /> For sale</Badge>
                  )}
                  <h1 className="font-display text-3xl font-bold drop-shadow sm:text-5xl">{ride.name}</h1>
                  ...
                  {(ride.city || ride.region) && (
                    <p className="mt-1 inline-flex items-center gap-1 text-sm opacity-80">
                      <MapPin className="h-3.5 w-3.5" /> {[ride.city, ride.region].filter(Boolean).join(", ")}
                    </p>
                  )}
                </div>
      ...
      <div className="container mx-auto grid gap-8 px-4 py-8 lg:grid-cols-[1fr_300px]">
        <div className="space-y-8">
          ...
          {linkedListing && linkedListing.status !== "sold" && (
            <section className="rounded-xl border border-primary/30 bg-primary/5 p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-primary">For sale</p>
                  <h3 className="font-display text-lg font-semibold">{linkedListing.title}</h3>
                  <p className="text-sm text-muted-foreground">{formatPHP(linkedListing.price_php)}</p>
                </div>
                <Button asChild><Link to="/listing/$id" params={{ id: linkedListing.id }}>View listing <ExternalLink className="ml-1 h-3.5 w-3.5" /></Link></Button>
              </div>
            </section>
          )}
          {linkedListing && linkedListing.status === "sold" && (
          ...
          {photos.length > 0 && (
            <section>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {photos.map((p) => (
                  <a key={p.id} href={p.url} target="_blank" rel="noreferrer" className="aspect-square overflow-hidden rounded-lg border border-border">
                    <img src={p.url} alt={p.caption ?? ""} loading="lazy" className="h-full w-full object-cover transition-transform hover:scale-105" />
                  </a>
                ))}
              </div>
            </section>
          ...
          {/* Specs */}
          <section>
            <h2 className="mb-3 font-display text-xl font-semibold flex items-center gap-2"><Gauge className="h-5 w-5" /> Specifications</h2>
            <dl className="grid gap-x-6 gap-y-2 rounded-xl border border-border bg-card p-4 sm:grid-cols-2">
              {specs.filter(([, v]) => v).map(([k, v]) => (
                <div key={k} className="flex justify-between border-b border-border/50 py-1.5 text-sm">
          ...
          {mods.length > 0 && (
            <section>
              <h2 className="mb-3 font-display text-xl font-semibold flex items-center gap-2"><Settings className="h-5 w-5" /> Modifications</h2>
              <div className="space-y-4">
                {Object.entries(modsByCat).map(([cat, items]) => (
                  <div key={cat} className="rounded-xl border border-border bg-card p-4">
                    <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-primary">{cat.replace(/_/g, " ")}</h3>
                    <ul className="divide-y divide-border">
                      {items.map((m) => (
                        <li key={m.id} className="grid grid-cols-1 gap-1 py-2 sm:grid-cols-[1fr_auto] sm:items-center">
                          <div>
                            <p className="font-medium">{m.part_name}</p>
                            {(m.brand || m.notes) && (
                              <p className="text-xs text-muted-foreground">{[m.brand, m.notes].filter(Boolean).join(" · ")}</p>
                            )}
                          </div>
                  ...
                  <li key={s.id} className="mb-5 last:mb-0">
                    ...
                    <div className="rounded-lg border border-border bg-card p-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">{formatDate(s.service_date)}</span>
                      </div>
                ...
                {ownership.map((o) => (
                  <li key={o.id} className="grid grid-cols-1 gap-1 p-3 sm:grid-cols-[1fr_auto]">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {o.acquired_on ? formatDate(o.acquired_on) : "?"} → {o.sold_on ? formatDate(o.sold_on) : "present"}
                    </p>
                  </li>
                ))}
              </ul>
        ...
        <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
          <Link to="/seller/$id" params={{ id: ride.user_id }} className="block rounded-xl border border-border bg-card p-4 hover:shadow-md">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-secondary">
                {ownerLogo ? <img src={ownerLogo} alt="" className="h-full w-full object-cover" /> : <span className="text-lg font-semibold">{ownerName.charAt(0)}</span>}
              </div>
              <div>
