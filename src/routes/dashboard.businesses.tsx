  ...
  if (!editing) {
    return (
      ...
      >
        ...
        {row.price_label ? (
          <span className="font-medium text-foreground">{row.price_label}</span>
        ) : (
          "Add price / rate"
        )}
...
function tierBadge(tier: Row["subscription_tier"]) {
  ...
  return (
    <Badge className={styles[tier] ?? ""}>{tier.charAt(0).toUpperCase() + tier.slice(1)}</Badge>
  );
}
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await (supabase as any)
        .from("businesses")
        .select(
          "id,slug,name,type_slug,status,city,region,rating_avg,rating_count,price_label,subscription_tier,logo_url,cover_url,featured_video_url,description,hours,phone,vanity_slug",
        )
        .eq("owner_id", user.id)
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <PriceEditor
                      ...
                      onSaved={(val) =>
                        setRows((prev) =>
                          prev.map((r) => (r.id === b.id ? { ...r, price_label: val } : r)),
                        )
                      }
                    />
                  </div>
                  ...
                  {b.status === "active" && (
                    <>
                      <Button size="sm" variant="outline" onClick={() => setPlanTarget(b)}>
                        ...
                        {b.subscription_tier && b.subscription_tier !== "free"
                          ? "Change plan"
                          : "Upgrade"}
                      </Button>
                      <ShareQr
