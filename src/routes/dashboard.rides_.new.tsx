  ...
  return (
    <form onSubmit={submit} className="mx-auto max-w-2xl space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Start with the basics — you can add photos, mods and history next.</p>
      </div>
      <div className="space-y-2">
        <Label>Nickname</Label>
        <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Project Stancekipo" />
      </div>
      <div className="space-y-2">
        <Label>Vehicle type</Label>
        <div className="flex gap-2">
          {(["car", "motorcycle"] as const).map((t) => (
            <Button key={t} type="button" variant={vehicleType === t ? "default" : "outline"} size="sm" onClick={() => setVehicleType(t)}>
      ...
      <div className="flex justify-end gap-2">
        <Button type="submit" disabled={busy}>{busy ? "Creating…" : "Create ride"}</Button>
      </div>
    </form>
  );
}
