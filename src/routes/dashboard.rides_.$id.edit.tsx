...
function EditRidePage() {
  ...
  const load = async () => {
    ...
    const [{ data: p }, { data: m }, { data: l }, { data: o }, { data: ls }] = await Promise.all([
      ...
      supabase
        ...
        .eq("user_id", user.id)
        .in("status", ["draft", "active", "pending_sale"]),
    ]);
    setPhotos(p ?? []);
    setMods(m ?? []);
    setLogs(l ?? []);
  ...
  const delMod = async (mid: string) => {
    await (supabase as any).from("ride_mods").delete().eq("id", mid);
    setMods(mods.filter((x) => x.id !== mid));
  };
  ...
  const delLog = async (lid: string) => {
    await (supabase as any).from("ride_service_log").delete().eq("id", lid);
    setLogs(logs.filter((x) => x.id !== lid));
  };
  ...
  const delOwner = async (oid: string) => {
    await (supabase as any).from("ride_ownership").delete().eq("id", oid);
    setOwners(owners.filter((x) => x.id !== oid));
  };

  const togglePublish = async () => {
    const isPub = ride.status === "published";
    try {
      await publish({ data: { id, publish: !isPub } });
  ...
  const updateListingLink = async (lid: string) => {
    try {
      await linkListing({ data: { ride_id: id, listing_id: lid || null } });
      toast.success("Saved");
      load();
    } catch (e: any) {
      toast.error(e.message);
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <Button
            ...
            variant={ride.status === "published" ? "outline" : "default"}
          >
      ...
      <Tabs defaultValue="details">
              ...
              <div key={k} className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setRide({
                      ...ride,
                      [k]: k === "year" || k === "mileage_km" ? (v ? Number(v) : null) : v,
                    });
                  }}
                />
              </div>
        ...
        <TabsContent value="photos" className="pt-4">
          <RidePhotoUploader
            ...
            userId={user!.id}
            photos={photos}
            coverUrl={ride.cover_photo_url}
            onChange={load}
          />
        </TabsContent>

        <TabsContent value="mods" className="space-y-3 pt-4">
          <Button onClick={addMod} variant="outline" size="sm">
          ...
          {mods.map((m, i) => (
            ...
            >
              <select
                className="rounded-md border border-input bg-background px-2 py-1 text-sm sm:col-span-2"
                value={m.category}
                onChange={(e) => {
                  const v = { ...m, category: e.target.value };
                  setMods(mods.map((x, j) => (j === i ? v : x)));
                  saveMod(v);
                }}
              >
              ...
              <Input
                ...
                value={m.part_name ?? ""}
                onChange={(e) =>
                  setMods(mods.map((x, j) => (j === i ? { ...x, part_name: e.target.value } : x)))
                }
                onBlur={() => saveMod(m)}
              ...
              <Input
                ...
                value={m.brand ?? ""}
                onChange={(e) =>
                  setMods(mods.map((x, j) => (j === i ? { ...x, brand: e.target.value } : x)))
                }
                onBlur={() => saveMod(m)}
              />
              <Input
                ...
                onChange={(e) =>
                  setMods(
                    mods.map((x, j) =>
                      j === i
                        ? { ...x, cost_php: e.target.value ? Number(e.target.value) : null }
                        : x,
                    ),
                  )
                }
              ...
              <Input
                type="date"
                value={m.installed_on ?? ""}
                onChange={(e) =>
                  setMods(
                    mods.map((x, j) =>
                      j === i ? { ...x, installed_on: e.target.value || null } : x,
                    ),
                  )
                }
                onBlur={() => saveMod(m)}
              ...
              <Input
                ...
                value={m.notes ?? ""}
                onChange={(e) =>
                  setMods(mods.map((x, j) => (j === i ? { ...x, notes: e.target.value } : x)))
                }
                onBlur={() => saveMod(m)}
          ...
          {logs.map((s, i) => (
            ...
            >
              <Input
                ...
                onChange={(e) =>
                  setLogs(
                    logs.map((x, j) => (j === i ? { ...x, service_date: e.target.value } : x)),
                  )
                }
                onBlur={() => saveLog(s)}
              />
              <Input
                ...
                onChange={(e) =>
                  setLogs(
                    logs.map((x, j) => (j === i ? { ...x, service_type: e.target.value } : x)),
                  )
                }
                onBlur={() => saveLog(s)}
                className="sm:col-span-2"
              ...
              <Input
                ...
                onChange={(e) =>
                  setLogs(
                    logs.map((x, j) =>
                      j === i
                        ? { ...x, mileage_km: e.target.value ? Number(e.target.value) : null }
                        : x,
                    ),
                  )
                }
              ...
              <Textarea
                ...
                value={s.notes ?? ""}
                onChange={(e) =>
                  setLogs(logs.map((x, j) => (j === i ? { ...x, notes: e.target.value } : x)))
                }
                onBlur={() => saveLog(s)}
          ...
          {owners.map((o, i) => (
            ...
            >
              <Input
                ...
                onChange={(e) =>
                  setOwners(
                    owners.map((x, j) => (j === i ? { ...x, owner_name: e.target.value } : x)),
                  )
                }
                onBlur={() => saveOwner(o)}
                className="sm:col-span-2"
              ...
              <Input
                ...
                onChange={(e) =>
                  setOwners(
                    owners.map((x, j) =>
                      j === i ? { ...x, acquired_on: e.target.value || null } : x,
                    ),
                  )
                }
                onBlur={() => saveOwner(o)}
              ...
              <Input
                ...
                onChange={(e) =>
                  setOwners(
                    owners.map((x, j) => (j === i ? { ...x, sold_on: e.target.value || null } : x)),
                  )
                }
                onBlur={() => saveOwner(o)}
              />
              ...
              <Input
                ...
                value={o.notes ?? ""}
                onChange={(e) =>
                  setOwners(owners.map((x, j) => (j === i ? { ...x, notes: e.target.value } : x)))
                }
                onBlur={() => saveOwner(o)}
        ...
        <TabsContent value="listing" className="space-y-3 pt-4">
          <p className="text-sm text-muted-foreground">
            Connect this ride to one of your active marketplace listings, or unlink it.
          </p>
          <select
...
function RideEngineField({ ride, setRide }: { ride: any; setRide: (r: any) => void }) {
  const cat: "car" | "motorcycle" =
    ride?.vehicle_type === "motorcycle" || ride?.vehicle_type === "scooter" ? "motorcycle" : "car";
  ...
  return (
    <div className="space-y-1 sm:col-span-2">
      ...
      >
        <SelectTrigger>
          <SelectValue placeholder={ride?.model ? "Select engine" : "Pick a model first"} />
        </SelectTrigger>
