...
const CreateInput = z.object({
  ...
  vehicle_type: z
    .enum(["car", "truck", "suv", "van", "motorcycle", "scooter", "atv", "utv", "boat", "other"])
    .default("car"),
});
...
export const createRide = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const base =
      slugify([data.year, data.make, data.model, data.name].filter(Boolean).join(" ")) || "ride";
...
export const toggleRideLike = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabase
      .from("ride_likes" as never)
      .insert({ ride_id: data.ride_id, user_id: userId } as never);
    if (error) throw new Error(error.message);
    return { liked: true };
  });
...
export const linkRideToListing = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabase
      .from("rides" as never)
      .update({
        linked_listing_id: data.listing_id,
        is_for_sale: data.listing_id ? true : false,
      } as never)
      .eq("id", data.ride_id);
    if (error) throw new Error(error.message);
    return { ok: true };
