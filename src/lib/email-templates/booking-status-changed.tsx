...
const BookingStatusChanged = ({
  ...
  <Html lang="en">
    <Head />
    <Preview>
      {business_name ?? "Your booking"} — {status ?? "update"}
    </Preview>
    <Body style={main}>
      <Container style={container}>
        ...
        <Text style={text}>
          ...
          {status === "cancelled" &&
            `${business_name} cancelled your booking. Reach out to reschedule if you'd like.`}
          {status === "completed" && `${business_name} marked your visit as completed.`}
          {status === "no_show" &&
            `${business_name} marked your appointment as a no-show. Contact them if this was a mistake.`}
        </Text>
        <Section style={card}>
        ...
        {business_slug && (
          <Text style={text}>
            ...
            <Link href={`${SITE_URL}/businesses/${business_slug}`}>
              {SITE_URL}/businesses/{business_slug}
            </Link>
          </Text>
        )}
        <Text style={footer}>
