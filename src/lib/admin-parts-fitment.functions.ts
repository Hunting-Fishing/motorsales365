...
export const adminListPartsFitmentWorkbench = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [catalog, productNumbers, profiles, fitments] = await Promise.all([
      supabaseAdmin
        .from("parts_catalog")
        ...
        .limit(500),
      supabaseAdmin
        .from("parts_product_numbers")
        .select(
          "id,product_id,number,normalized_number,number_type,manufacturer,country_code,is_primary,source,source_reference,active,parts_catalog(title,manufacturer,manufacturer_part_number)",
        )
      ...
      supabaseAdmin
        ...
        .select(
          "id,country_code,make,model,variant,year_min,year_max,engine_code,chassis_code,status,source",
        )
        .order("make")
        .order("model")
        .limit(1000),
      supabaseAdmin
        .from("parts_fitment")
        .select(
          "id,product_id,vehicle_profile_id,position,fitment_status,source,source_reference,confidence,reviewed_at,parts_catalog(title,manufacturer_part_number),parts_vehicle_profiles(make,model,variant,year_min,year_max,engine_code,chassis_code)",
        )
        .order("updated_at", { ascending: false })
        .limit(500),
    ]);
    if (catalog.error) throw new Error(catalog.error.message);
    if (productNumbers.error) throw new Error(productNumbers.error.message);
    if (profiles.error) throw new Error(profiles.error.message);
    if (fitments.error) throw new Error(fitments.error.message);
    return {
      catalog: catalog.data ?? [],
      productNumbers: productNumbers.data ?? [],
...
export const adminUpsertPartsProductNumber = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => ProductNumberSchema.parse(input))
...
export const adminDeletePartsProductNumber = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ id: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
    await requireAdmin(context);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await supabaseAdmin.from("parts_product_numbers").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
