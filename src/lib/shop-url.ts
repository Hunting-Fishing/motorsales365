...
const HOST_PATTERNS: Array<{ slug: string; test: (host: string, url: URL) => boolean }> = [
  {
    slug: "shopee",
    test: (h) => /(^|\.)shopee\.(ph|com|sg|com\.my|co\.th|vn|co\.id|com\.br)$/i.test(h),
  },
  { slug: "lazada", test: (h) => /(^|\.)lazada\.(com\.ph|com\.my|sg|co\.th|vn|co\.id)$/i.test(h) },
  {
    slug: "tiktok",
    test: (h, u) => /(^|\.)tiktok\.com$/i.test(h) && /\/shop|\/product/.test(u.pathname),
  },
  { slug: "tiktok", test: (h) => /(^|\.)(vt|vm)\.tiktok\.com$/i.test(h) },
  {
    slug: "amazon",
    test: (h) =>
      /(^|\.)amazon\.(com|co\.uk|de|fr|it|es|ca|com\.au|sg|ae|in|co\.jp|nl|se|pl)$/i.test(h),
  },
