...
- 2026-06-08 — Item #13 VIN scanner. New `VinScanDialog` component (camera + paste). Camera path uses the browser's `BarcodeDetector` API (Code 39/128, QR, Data Matrix) against the rear camera via `getUserMedia`; finds a VIN-shaped substring in any decoded barcode and stops automatically. Paste path validates VIN format (11–17 chars, no I/O/Q) and runs an **ISO 3779 mod-11 check-digit** on 17-char VINs (`vinChecksumValid` exported from the dialog and re-used inside `VehicleQualitySchema`). On a valid VIN the dialog calls NHTSA vPIC `DecodeVinValues` and auto-fills year/make/model, plus fuel and transmission when the API returns them. Wired as `vinScanSlot` next to the VIN input in both `/sell` and `/listing/$id/edit` — parent forms receive the decoded payload and update vehicle picker + VIN field together. If decode fails, the VIN is still saved and the seller is asked to fill the rest manually.
- 2026-06-08 — Item #18 done (Reviews & Reputation). New `seller_reviews` table (seller_id, reviewer_id, optional listing_id, rating 1–5, body ≤ 2000 chars, transaction_completed flag, status active/hidden/removed). RLS: anon can read active reviews, authenticated can write one review per (seller,listing) and edit/delete only their own (CHECK blocks self-reviews); moderators manage all. Trigger `seller_reviews_after_change` keeps new `profiles.seller_rating_avg` + `seller_rating_count` + `reviews_updated_at` in sync on every insert/update/delete. `public_profiles` view now exposes the aggregate so it's readable by anon. New `SellerReputationBadges` component derives badges from existing profile data: **365 Verified** (verification_status='verified'), **FB Verified** (fb_verified_at), **Top Rated** (avg ≥ 4.5 with ≥ 5 reviews), **Repeat Seller** (≥ 5 active listings or ≥ 3 sold), **Verified Transaction** (≥ 1 completed sale), **Founding Member** (is_founding_member). New `SellerReviews` component (5-star picker + textarea, single review per buyer, edit/delete own, public list with reviewer name/avatar). `/seller/$id` now shows a rating chip in the hero, the reputation badge row, and a new **Reviews** tab. Future badges (Responds Fast, Documents Checked, Inspection Completed) tracked as `FUTURE_BADGES` stub for when response-time / OR-CR / inspection-partner data lands.
- 2026-06-09 — Sync-groups registry + Item #14 Vehicle Passport extended. New `.lovable/sync-groups.md` registry; every passport file now carries a `SYNC GROUP: vehicle-passport` header (`src/routes/passport.$slug.tsx`, `src/routes/dashboard.vehicles.tsx`, `src/lib/vehicles.functions.ts`, `src/components/passport-share-section.tsx`). Migration added `ownership_count`, `disclosures` (jsonb: flood/accident level + notes), `modifications`, `transferred_to_listing_id`, `passport_premium` to `vehicles`; new `vehicle_photos` table (anon read when parent vehicle is_public, owner CRUD via RLS). `vehicles.functions.ts` zod schema and create/update now persist the new fields; `getMyVehicle` returns photos. Public `/passport/$slug` loader fetches photos in parallel and renders new Ownership + Disclosures cards, Modifications section, and Photos gallery. Dashboard vehicle dialog gained Passport Disclosures block (ownership #, flood, accident, notes, modifications) plus existing public toggle + QR share.
- 2026-06-09 — Item #20 done (Inspection & transaction-safety upsells). New `inspection_services` catalog (slug, name, description, category, PHP price range, pricing_unit, active) seeded with 6 starter offers (OR/CR review ₱199–499, ID verify ₱99–299, pre-purchase inspection ₱500–2,500, mechanic-booking commission, vehicle history report ₱199–999, transaction assistance %). New `inspection_orders` table (buyer_id, service_id, listing_id, contact, region, preferred_date, notes, status requested|assigned|in_progress|completed|cancelled, optional provider_id/payment_id). RLS: anon can read active catalog; authenticated can create their own orders and update while requested|assigned; admin/support manage all. New `src/lib/inspection.functions.ts` exposes `listInspectionServices`, `createInspectionOrder` (auth-gated), `listMyInspectionOrders`. New public route `/services/inspection` with rate-card grid + linked request form (pre-fills from `?service=&listing=` query, redirects to /login on submit when signed-out). Listing detail (`/listing/$id`) gains a "Before you pay — buyer safety add-ons" aside (cars/motos/trucks only) with "Request inspection" / "See the rate card" CTAs. Sync-groups registry entry added; Terms §11a "Optional inspection & transaction-safety services" added (no "escrow" wording — uses "payment release partner") and `Refund Policy` gained matching section. Both bumped to June 9, 2026.
