-- Phase 1: Create missing database tables to replace mock data (simplified RLS)
...
CREATE TABLE IF NOT EXISTS public.form_submissions (
  id TEXT NOT NULL DEFAULT gen_random_uuid()::text PRIMARY KEY,
  form_id TEXT REFERENCES public.custom_forms(id) ON DELETE CASCADE,
  customer_id TEXT,
  work_order_id TEXT,
...
-- Create simple RLS policies for authenticated users (will refine later)
CREATE POLICY "Authenticated users can access team members" ON public.team_members FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can access affiliate tools" ON public.affiliate_tools FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can access equipment" ON public.equipment FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can access forms" ON public.custom_forms FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can access form submissions" ON public.form_submissions FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can access equipment maintenance" ON public.equipment_maintenance FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can access feedback" ON public.customer_feedback FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can access analytics" ON public.system_analytics FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can access performance metrics" ON public.performance_metrics FOR ALL USING (auth.uid() IS NOT NULL);
