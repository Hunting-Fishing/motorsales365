...
-- Maintenance schedules for equipment/vehicle maintenance (fixed equipment reference)
...
-- Tool reviews for affiliate tools
CREATE TABLE public.tool_reviews (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tool_id UUID REFERENCES affiliate_tools(id) ON DELETE CASCADE NOT NULL,
