-- Phase 7: Public Portal, Raffle Management & Donor Tools (Fixed)
...
-- Nonprofit donors table (different from existing donations table)
CREATE TABLE public.nonprofit_donors (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    shop_id UUID NOT NULL,
...
-- Nonprofit donations table
CREATE TABLE public.nonprofit_donations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    shop_id UUID NOT NULL,
    donor_id UUID REFERENCES public.nonprofit_donors(id),
...
CREATE INDEX idx_nonprofit_donors_shop_id ON public.nonprofit_donors(shop_id);
CREATE INDEX idx_nonprofit_donors_type ON public.nonprofit_donors(donor_type);
CREATE INDEX idx_nonprofit_donations_shop_id ON public.nonprofit_donations(shop_id);
CREATE INDEX idx_nonprofit_donations_donor ON public.nonprofit_donations(donor_id);
CREATE INDEX idx_nonprofit_donations_date ON public.nonprofit_donations(donation_date);
...
CREATE TRIGGER update_nonprofit_donors_updated_at
    BEFORE UPDATE ON public.nonprofit_donors
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();
...
ALTER TABLE public.nonprofit_donors ENABLE ROW LEVEL SECURITY;
...
-- RLS Policies for nonprofit donors/donations/sponsors (staff only)
CREATE POLICY "Staff can manage nonprofit donors" ON public.nonprofit_donors
    FOR ALL USING (shop_id IN (SELECT shop_id FROM profiles WHERE id = auth.uid()));

CREATE POLICY "Staff can manage nonprofit donations" ON public.nonprofit_donations
    FOR ALL USING (shop_id IN (SELECT shop_id FROM profiles WHERE id = auth.uid()));
