-- Phase 5: Complete Non-Profit Management System
...
CREATE OR REPLACE FUNCTION public.get_volunteer_statistics(shop_id_param uuid)
RETURNS TABLE (
  total_volunteers integer,
  active_volunteers integer,
  total_hours_this_month numeric,
  average_hours_per_volunteer numeric
)
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  SELECT 
    ...
    COUNT(CASE WHEN v.status = 'active' THEN 1 END)::integer as active_volunteers,
    COALESCE(SUM(CASE 
      WHEN vh.activity_date >= date_trunc('month', CURRENT_DATE) 
      THEN vh.hours_worked 
      ELSE 0 
    END), 0) as total_hours_this_month,
    CASE 
      WHEN COUNT(CASE WHEN v.status = 'active' THEN 1 END) > 0 
      THEN v.total_hours_logged / COUNT(CASE WHEN v.status = 'active' THEN 1 END)::numeric
      ELSE 0 
    END as average_hours_per_volunteer
  FROM public.volunteers v
  LEFT JOIN public.volunteer_hours vh ON v.id = vh.volunteer_id
  WHERE v.shop_id = shop_id_param;
END;
$$;

CREATE OR REPLACE FUNCTION public.get_fundraising_progress(shop_id_param uuid)
RETURNS TABLE (
  total_goal numeric,
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(CASE WHEN status = 'active' THEN 1 END)::integer as active_campaigns,
    CASE 
      WHEN SUM(goal_amount) > 0 
      THEN (SUM(raised_amount) / SUM(goal_amount) * 100)
      ELSE 0 
...
CREATE OR REPLACE FUNCTION public.get_program_impact_summary(shop_id_param uuid)
RETURNS TABLE (
  total_programs integer,
  active_programs integer,
  total_participants integer,
  programs_with_metrics integer
)
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
  SELECT 
    COUNT(*)::integer as total_programs,
    COUNT(CASE WHEN p.status = 'active' THEN 1 END)::integer as active_programs,
    COALESCE(SUM(p.participants_served), 0)::integer as total_participants,
    COUNT(DISTINCT im.program_id)::integer as programs_with_metrics
  FROM public.programs p
  LEFT JOIN public.impact_metrics im ON p.id = im.program_id
