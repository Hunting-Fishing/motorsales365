-- Phase 7.2: Fix Database Function Type Issues and Add Missing Functions (Fixed)

-- Drop existing functions completely first
DROP FUNCTION IF EXISTS public.get_popular_products(integer, integer);
DROP FUNCTION IF EXISTS public.get_product_interactions_by_category();
DROP FUNCTION IF EXISTS public.get_recently_viewed_products(uuid, text, integer);
