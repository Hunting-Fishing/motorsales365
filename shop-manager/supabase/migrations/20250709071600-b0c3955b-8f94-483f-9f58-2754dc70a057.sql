-- Phase 8B: Advanced Integration Features and Automation (Fixed)
...
-- Webhook retry queue (without FK to webhooks for now)
CREATE TABLE IF NOT EXISTS public.webhook_retry_queue (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  webhook_id uuid, -- Will add FK constraint later when webhooks table exists
...
CREATE INDEX IF NOT EXISTS idx_integration_workflows_next_run_at ON public.integration_workflows(next_run_at) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_integration_analytics_time_bucket ON public.integration_analytics(time_bucket);

CREATE INDEX IF NOT EXISTS idx_webhook_retry_queue_status ON public.webhook_retry_queue(status);
CREATE INDEX IF NOT EXISTS idx_webhook_retry_queue_next_retry_at ON public.webhook_retry_queue(next_retry_at) WHERE status = 'pending';
...
CREATE INDEX IF NOT EXISTS idx_sync_queue_priority ON public.sync_queue(priority, created_at);
...
CREATE POLICY "Shop members can view workflows" ON public.integration_workflows
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Shop members can view webhook retry queue" ON public.webhook_retry_queue
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.integration_webhooks w
      JOIN public.shop_integrations si ON si.id = w.integration_id
      JOIN public.profiles p ON p.shop_id = si.shop_id
      WHERE w.id = webhook_retry_queue.webhook_id AND p.id = auth.uid()
    )
  );
