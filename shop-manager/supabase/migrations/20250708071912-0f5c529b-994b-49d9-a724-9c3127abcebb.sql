-- Phase 1: Essential Database Structure Fixes (Final)
...
-- Add varied statuses using a simpler approach
UPDATE work_orders 
SET status = 'completed'
WHERE SUBSTRING(id::text, 1, 1) IN ('0', '1', '2', '3');

UPDATE work_orders 
SET status = 'in_progress'
WHERE SUBSTRING(id::text, 1, 1) IN ('4', '5', '6', '7') AND status != 'completed';

UPDATE work_orders 
SET status = 'pending'
WHERE SUBSTRING(id::text, 1, 1) IN ('8', '9', 'a', 'b') AND status NOT IN ('completed', 'in_progress');

UPDATE work_orders 
SET status = 'assigned'
WHERE status NOT IN ('completed', 'in_progress', 'pending');
