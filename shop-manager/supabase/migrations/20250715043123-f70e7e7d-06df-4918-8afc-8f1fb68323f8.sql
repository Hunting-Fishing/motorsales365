-- Phase 1: Create missing database tables for non-profit management
...
CREATE TABLE public.impact_metrics (
    ...
    name TEXT NOT NULL,
    description TEXT,
    metric_type TEXT NOT NULL CHECK (metric_type IN ('quantitative', 'qualitative', 'financial', 'social')),
    unit_of_measurement TEXT,
    target_value NUMERIC,
    current_value NUMERIC DEFAULT 0,
    reporting_frequency TEXT DEFAULT 'monthly' CHECK (reporting_frequency IN ('daily', 'weekly', 'monthly', 'quarterly', 'annually')),
    program_id UUID,
    shop_id UUID NOT NULL,
...
ADD CONSTRAINT impact_metrics_shop_id_fkey FOREIGN KEY (shop_id) REFERENCES public.shops(id) ON DELETE CASCADE,
ADD CONSTRAINT impact_metrics_program_id_fkey FOREIGN KEY (program_id) REFERENCES public.programs(id) ON DELETE CASCADE,
ADD CONSTRAINT impact_metrics_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id) ON DELETE CASCADE;
