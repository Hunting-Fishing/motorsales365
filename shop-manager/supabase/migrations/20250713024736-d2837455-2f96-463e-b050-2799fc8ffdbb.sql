-- Phase 5: AI & Automation Tables (Fixed)
...
-- RLS Policies for AI Chat Sessions (Fixed to use correct role names)
CREATE POLICY "Users can view chat sessions" ON public.ai_chat_sessions FOR SELECT USING (
    user_id = auth.uid() OR 
    EXISTS (SELECT 1 FROM user_roles ur JOIN roles r ON r.id = ur.role_id 
            WHERE ur.user_id = auth.uid() AND r.name IN ('admin', 'reception'))
);
