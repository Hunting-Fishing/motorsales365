-- Fix get_user_effective_permissions to correctly map auth.uid() to profile.id
-- The _user_id parameter is auth.uid(), but user_roles and user_permissions store profile.id
...
BEGIN
  ...
  -- Get user's role name using profile_id (user_roles stores profile.id, not auth.uid())
  ...
  -- Get user-specific permission overrides using profile_id
  SELECT up.actions INTO _user_overrides
  FROM user_permissions up
  WHERE up.user_id = _profile_id
