-- Phase 1: Create missing database tables to replace mock data (fixed RLS)
...
CREATE POLICY "Users can access team members from their shop" ON public.team_members
  FOR ALL USING (shop_id IN (SELECT shop_id FROM profiles WHERE id = auth.uid()::text));
...
CREATE POLICY "Users can access form submissions from their shop" ON public.form_submissions
  FOR ALL USING (form_id IN (SELECT id FROM custom_forms WHERE shop_id IN (SELECT shop_id FROM profiles WHERE id = auth.uid()::text)));

CREATE POLICY "Users can access equipment maintenance from their shop" ON public.equipment_maintenance
  FOR ALL USING (equipment_id IN (SELECT id FROM equipment WHERE shop_id IN (SELECT shop_id FROM profiles WHERE id = auth.uid()::text)));
