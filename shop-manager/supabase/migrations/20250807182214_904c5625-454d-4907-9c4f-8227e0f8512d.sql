...
-- Initialize security_settings for all shops (fixed ip_whitelist type)
...
SELECT DISTINCT
  ...
  ARRAY[]::text[] as ip_whitelist -- Fixed: Empty text array instead of string
FROM profiles p
WHERE p.shop_id IS NOT NULL
  AND NOT EXISTS (
...
ALTER TABLE appearance_settings REPLICA IDENTITY FULL;
ALTER TABLE branding_settings REPLICA IDENTITY FULL;
ALTER TABLE security_settings REPLICA IDENTITY FULL;
ALTER TABLE unified_settings REPLICA IDENTITY FULL;