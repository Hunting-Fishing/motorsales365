...
import { Loader2, UserPlus, Droplets, CheckCircle } from 'lucide-react';

export default function WaterDeliveryPortalRegister() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.auth.signUp({
        email, password,
        options: { emailRedirectTo: `${window.location.origin}/water-delivery-portal/login`, data: { first_name: firstName, last_name: lastName } }
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-muted/30 px-4">
      <Card className="w-full max-w-md shadow-xl">
        <CardContent className="pt-8 pb-6 text-center space-y-4">
          ...
          <Link to="/water-delivery-portal/login"><Button className="w-full bg-cyan-700 hover:bg-cyan-800 text-white mt-4">Go to Sign In</Button></Link>
        </CardContent>
      </Card>
    </div>
  );
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="shadow-xl bg-card/95 backdrop-blur-sm border-border/50">
          <CardHeader className="text-center space-y-2">
            <div className="mx-auto w-16 h-16 bg-cyan-700 rounded-full flex items-center justify-center mb-2"><UserPlus className="h-8 w-8 text-white" /></div>
            <CardTitle className="text-2xl font-bold">Create Account</CardTitle>
            <CardDescription>Register to access your water delivery portal</CardDescription>
          </CardHeader>
          <form onSubmit={handleRegister}>
            ...
            <CardFooter className="flex flex-col gap-4">
              ...
              <div className="text-center text-sm text-muted-foreground">
                ...
                <Link to="/water-delivery-portal/login" className="text-cyan-700 hover:text-cyan-900 dark:text-cyan-400 dark:hover:text-cyan-300 font-medium">Sign in</Link>
              </div>
            </CardFooter>
          </form>
        </Card>
