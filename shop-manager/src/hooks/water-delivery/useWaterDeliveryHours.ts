...
export interface WaterDeliveryHour {
  id?: string;
  shop_id: string;
  day_of_week: number;
  open_time: string;
...
export function useWaterDeliveryHours(shopId: string | null) {
  const queryClient = useQueryClient();

  const query = useQuery({
    queryKey: ['water-delivery-hours', shopId],
    queryFn: async () => {
      if (!shopId) return [];
      
      const { data, error } = await supabase
        .from('water_delivery_hours')
        .select('*')
        .eq('shop_id', shopId)
        .order('day_of_week');
      
      if (error) throw error;
      return data as WaterDeliveryHour[];
    },
    enabled: !!shopId,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (hours: WaterDeliveryHour[]) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['water-delivery-hours', shopId] });
